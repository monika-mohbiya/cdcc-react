...
export default function AnsKey() {
  ...
  // const questionBaseurl = import.meta.env.VITE_API_URL + '/uploads/questions/' || ''
  ...
  const optionBaseurl = Object.values(optionFiles);

  function getQuestionImage(papershiftid, qid) {
    return questionBaseurl.find(url =>
      url.endsWith(`/${papershiftid}/${qid}.png`)
    );
  }

  function getOptionImage(papershiftid, optid) {
    return optionBaseurl.find(url =>
      url.endsWith(`/${papershiftid}/${optid}.png`)
    );
  }
                          ...
                          <tr>
                            <td>
                              ...
                              {/* <img
                                ...
                                src={`${questionBaseurl}${question?.papershiftid}/${question?.q_id}.png`}
                                alt={`Question ${i + 1}`}
                              /> */}
                              <img
                                className="img-fluid img-bordered"
                                src={getQuestionImage(question?.papershiftid, question?.q_id)}
                                alt={`Question ${i + 1}`}
                              />
                                  ...
                                  <td>
                                    <div className="mb-1" style={optionDiv}>
                                      ...
                                      {/* <img
                                        ...
                                        src={`${optionBaseurl}${question?.papershiftid}/${option?.opt_id}.png`}
                                        alt={`Option ${getOptionLabel(j)}`}
                                      /> */}
                                      <img
                                        className={`img-fluid ${highlight}`}
                                        src={getOptionImage(question?.papershiftid, option?.opt_id)}
                                        alt={`Option ${getOptionLabel(j)}`}
                                      />
                                      <span></span>
                                    </div>
